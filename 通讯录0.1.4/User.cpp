...
UserNode* User::SearchUser_node(IN_PARAM string first)
{
    ...
    while(temp != NULL)
    {
        if(first == temp->str_name&&temp->b_in_blacklist==false)
        {
            Find_more = temp->next;
            int is_print = 0;//检测是否打印表头
            while (Find_more != NULL) {
                if (first == Find_more->str_name&&Find_more->b_in_blacklist==false) {
                    if (0 == is_print) {
        ...
        if (first == temp->str_phone_num&&temp->b_in_blacklist==false)
        {
            return temp;
        }
        temp = temp->next;
 ...
 void UserNode::Set_Last_Message(IN_PARAM string first)
{
 ...
 void User::Show_Usernode_With_Sort() {
     ...
     {
         if (((User_Iter->str_phone_num) > (U_Print_Ctrl->str_phone_num))&&User_Iter->b_in_blacklist==false )   //电话号码最大
         {
     ...
     while (!All_Printed())                           //先判断是否打印完，每次循环打印一个
     {
         ...
         {
             if (((User_Iter->str_phone_num) < (U_Print_Ctrl->str_phone_num ))&& !(User_Iter->b_is_printed)&&User_Iter->b_in_blacklist==false)   //电话号码更小且未打印
             {
 ...
 bool User::All_Printed()
 {
     ...
     {
         if (!(Uprint_Scan->b_is_printed)&&Uprint_Scan->b_in_blacklist==false ) //有没打印的
         {
             return false;               //就没打印完
         }
